...
progress: 97
blocked_on: none
next_step: "ACTIVE: the Block 1 evidence rebuild on branch `block1-evidence-rebuild`. Tasks 1-6 DONE — question catalog (30 program-decision questions), evidence pack builder, `main.py evidence` CLI, citation checker, leak-guard allow-list (83 tests pass), and the real pack built at `evidence/2026-08-11` (30/30 covered, gitignored; coverage report committed). KEY FINDING: coverage is not consensus — `deload_magnitude` filled its whole 12-passage budget from just 2 sources, all Torokhtiy, 10 from one article, so the live block's 'sets -40%' cannot be checked against agreement. Cite source counts, not passage counts. Task 7 DONE — `docs/programs/2026-08-11-block1-rebuild.md`, written blind from 16 pack files, 59 citations ALL RESOLVING via `python -m tools.check_citations`, plus a 10-item judgment ledger; leak guard and 83 tests pass. FINDINGS: (1) **TALL-LIFTER VALIDATION PASSED** — the blind query reproduced the v3.2.0 squat protocol from the same two independent sources (Max Aita E20.6, Sika E20.7/E20.10) plus Everett (E20.9), and added numbers the original lacked: positions hold at ~65-70% 1RM and <=3-5 reps, and adopting the correct pattern temporarily costs 30-40% of the old 1RM. (2) The corpus CONTRADICTS the block's founding premise — amateurs should not take a dedicated hypertrophy block, since 6-8 weeks costs ~3 months once detraining and re-acclimatization are counted [E1.2]; embed hypertrophy in weightlifting blocks instead [E1.1]. (3) OVERHEAD SQUATS SHOULD BE NEARLY ABSENT — 1-2x/week, 5-7 sets of 2-3, never standalone [E10.8], because heavy OHS progressions steal recovery from the snatch and jerk [E10.5] and the overhead is rarely a strength problem [E11.2]; the one prescribed use is a 5-second isometric hold [E11.3]. This INVERTS the shipped v3.5.2-3.5.6 change that ADDED OHS volume on master_synthesis.md's advice — exactly the class of error this project exists to catch. (4) The live pain-gate ('back pain >3/10 -> drop load ~40%') appears NOWHERE in the corpus — same shape as the archived synthesis's invented 'stop if pain >3/10', still shipping; needs a clinician, not a corpus. (5) '4-6 exercises per session' has zero support. (6) Everett prefers PRESCRIBED weights over daily maxes when training history exists [E16.7]. REMAINING: Task 8 — per-prescription diff vs PROGRAM_B1 with the uncovered fraction as the headline (PROGRAM_B1 may now be read; the blind phase is over). The live program stays untouched (athlete is in week 6 of 8). Use `python main.py ask \"<question>\"` as the source of truth for programming decisions — NOT master_synthesis.md, which is one 7-topic LLM view over ~5% of the corpus. Open items: (1) SECURITY — a live Telegram bot token sits in the Z840's journal (3 tokenized api.telegram.org URLs, Jul 11/13, httpx logging request URLs at INFO); silence that logger BEFORE rotating so the replacement doesn't land there too. (2) Persist the numbered context block next to master_synthesis.md — retrieval reorders near-tied results between runs, so citation numbers are not reproducible and cannot be audited after the fact. (3) Re-run `python main.py index` to pick up transcripts extracted since the migration (idempotent). (4) Deploy Brain_Dump's remaining 20-file drift (equipment feature + spool fixes) as its own deliberate change — the migration deployed only transcript persistence. Deferred: purge the compromised last_manorg corpus from this repo; master_synthesis.md's remaining recommendations (front squat primacy, belt squat volume, OHS load progression)"
updated: 2026-08-11
