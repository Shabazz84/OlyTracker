...
- **Cloud sync is Supabase only.** The GitHub Gist sync path was removed — `sbSync` (`docs/sync.js`, hand-written, not generated) auto-syncs sessions/sets/reviews on every mutation and pulls on startup. Set writes are local-first: each is queued in `localStorage.oly_sync_pending` until Supabase confirms it, the startup pull never overwrites a pending key, and pending writes retry on launch / reconnect / return-to-foreground — without this, sets logged mid-workout were wiped when Android killed the backgrounded PWA (v3.13.1). Tests: `node --test "tests/js/*.test.mjs"`. Don't reintroduce a second sync backend.
...
- Drill count scales inversely with skill: amateurs get many varied phase-broken-down drills (pulls by phase, transitions, footwork, jerks, presses separately) to build the movement in pieces; as mastery rises, drill count drops toward a minimum and full classic lifts replace them [`@athletists`, "Rhythm of Movement in Weightlifting. Tim Berestov", 2026-08-19 retrieval]
...
- The athlete's own body is the primary coach — a Berestov Team session shows a coach deliberately working at ~70% instead of forcing that day's planned max, "listen to it" [`@athletists`, "Berestov team", 2026-08-19 retrieval]

**Application:** 8-week block = Berestov's 2-month cycle. Lead-up exercises justify hang variations and positional work. "Build on correct patterns" is critical given back pain — compensatory patterns under load become permanent. The drill-scaling principle supports this project's own Phase 1→2 exercise trimming inside Block 1.

The rest of the principles above come from `berestovteam.ru` (web scrape, `summaries_archive/web_berestovteamru/`) and the Telegram export (`summaries_archive/telegram_atletisty/`) — both paths corrected 2026-08-19; they previously pointed at `summaries/...`, which doesn't exist.

**Data quality — resolved 2026-08-19.** The `@athletists` YouTube channel (61 videos) was originally extracted via YouTube auto-captions, not Whisper, and Claude's summary pass refused to synthesize a channel philosophy from it (flagged as corrupted/incoherent transcription). It was re-extracted with `--whisper` on 2026-07-28 (65/65 transcripts) and confirmed indexed and queryable via `python main.py ask` on 2026-08-19 — retrieval now returns coherent Russian-language coaching dialogue from real channel videos (the two principles above are cited from it). Those two bullets are the first citations pulled from it; treat the channel as available for further queries, not as a bulk-trusted source yet. Two artifacts remain stale and unrelated to this fix: `summaries_archive/athletists/channel_claude_summary.md` and `summaries_archive/athletists/channel_summary.UNRELIABLE_qwen_hallucination.md` were both generated by the pre-redesign per-channel summarizer (which baked the athlete profile into every summary — the bug Finding I1 fixed) and should not be used regardless of transcript quality.
